...
时间复杂度$O(n^2 \log a_i)$，维护不同的$\gcd$可以做到$O(n \log^2 a_i)$。
...
那么
$$
\mathbb{E}_{res}=\prod_{i=1}^na_i-\frac{k!}{n^k}\sum_{b_1+b_2+\cdots+b_n=k}\prod_{i=1}^n(a_i-b_i)
$$
注意到
$$
\begin{aligned}
\sum_{b_1+b_2+\cdots+b_n=k}\prod_{i=1}^n(a_i-b_i)&=[x^k]\prod_{i=1}^n\sum_{j=0}^k\frac{a_i-j}{j!}x^j \\
